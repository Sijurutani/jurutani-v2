  ...
  const startConversation = async () => {
    ...
    try {
      ...
      const { data: existingConversation } = await supabase
        ...
        .select('id')
        .or(
          `and(participant1_id.eq.${user.id || (user as any).sub},participant2_id.eq.${expert.value.user_id}),and(participant1_id.eq.${expert.value.user_id},participant2_id.eq.${user.id || (user as any).sub})`,
        )
        .single()
